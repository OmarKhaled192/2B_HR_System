...
export class ExecuseRequestComponent {
    constructor(
    ...
    ngOnInit() {
        ...
        Globals.getMainLangChanges().subscribe((mainLang) => {
            ...
            this.execuseRequestService.setCulture(mainLang);

            // update endpoint
            this.execuseRequestService.setEndPoint(this.endPoint);
    ...
    getDropDowns() {
        this.execuseRequestService.getDropdownField('Employee').subscribe({
            next: (res) => {
        ...
        this.execuseRequestService.getRequestTypeDropdown().subscribe({
            next: (res) => {
                console.log(res.data);
        ...
        this.execuseRequestService.getDropdownField('ExcuesType').subscribe({
            next: (res) => {
                console.log(res.data);
        ...
        this.execuseRequestService.getManagerDropdown().subscribe({
            next: (res) => {
                console.log(res.data);
    ...
    confirmDelete(rowData: any) {
        ...
        if (rowData.requestType === 0) {
            ...
            this.execuseRequestService
                .deleteExecuseRequestById(rowData.id)
                .subscribe({
                    next: (res) => {
                        // close dialog
    ...
    addNew(form: FormGroup) {
        form.patchValue({
            ...
            excuesTypeId: this.newMissonTypeId?.id ?? null,

            dateOfDay: this.DatePipe.transform(
                form.get('dateOfDay')?.value ?? null,
                'yyyy-MM-dd'
            ),
            startTime: form
                .get('startTime')
    ...
    loadData(
        ...
        this.execuseRequestService.GetPage(formData).subscribe({
            next: (res) => {
                console.log(res);
    ...
    confirmDeleteSelected() {
        ...
        this.execuseRequestService.DeleteRange(selectedIds).subscribe({
            next: (res) => {
    ...
    transformTime(time: string) {
        ...
        // Extract hours, minutes, and seconds from the original time string
        const [hours, minutes, secondsFraction] = time.split(':');
        ...
        date.setMilliseconds(milliseconds ? +milliseconds.slice(0, 3) : 0); // Take up to 3 digits for milliseconds
